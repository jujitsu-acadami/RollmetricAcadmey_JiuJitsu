...
import PerformanceLabPage from '../features/performanceLab/PerformanceLabPage';
import LoginPage from '../features/auth/LoginPage';
import { Page, Session, AppSettings, SessionState } from '../types';
import { bumpPerformanceLabAfterSession } from '../lib/performanceLab';
...
function AppContent() {
  ...
  const handleSessionEnd = (sessionData: Session) => {
    ...
    // Update Performance Lab progression + adherence score
    try {
      const resetCount = (sessionData as any)?.performanceLab?.resetCount ?? 0;
      bumpPerformanceLabAfterSession(sessionData, resetCount);
    } catch (e) {
      // non-blocking
      console.warn('Performance Lab update failed:', e);
    }
    ...
    switch (page) {
      ...
      case 'drill':
        return <DrillPage 
                ...
                />;
      case 'performance':
        return <PerformanceLabPage onNavigate={handleNavigate} />;
      case 'account':
