...
import { Session, Page, Drill, SessionState, AdvancedKpiType, PoseMetrics, EngineUpdateResult } from '../../types';
import { SettingsContext } from '../../contexts/SettingsContext';
import KpiPanel from '../../components/ui/KpiPanel';
import { startChatSession, getFeedbackStream } from '../../lib/geminiService';
import LoadingOverlay from '../../components/layout/LoadingOverlay';
import LiveView from './components/LiveView';
import FeedbackPanel from '../../components/ui/FeedbackPanel';
import { ALL_DRILLS, DRILL_CATEGORIES } from '../../config/DrillData';
import { BjjAnalyticsEngine } from '../../lib/BjjAnalyticsEngine';
import NoDrillsSelected from './components/NoDrillsSelected';
...
export type SessionTitleDisplayConfig = { type: 'title'; value: string; };
type SessionDropdownDisplayConfig = { type: 'dropdown'; label: string; items: string[]; };
export type SessionDisplayConfig = SessionTitleDisplayConfig | SessionDropdownDisplayConfig;
...
const MobileKpiSummary = ({ kpis }: { kpis: AdvancedKpiType }) => (
...
export default function DrillPage({ onSessionEnd, onNavigate, onSessionStateChange }: DrillPageProps) {
  ...
  useEffect(() => { feedbackLogRef.current = feedbackLog; }, [feedbackLog]);
  
  const isSessionActive = sessionState === 'running';

  useEffect(() => { analyticsEngine.current = new BjjAnalyticsEngine({}); }, []);
          ...
          <div className="hidden lg:flex absolute bottom-8 left-1/2 -translate-x-1/2 w-auto z-20">
            <div className="relative">
              <div className={`flex-shrink-0 bg-dd-surface/80 backdrop-blur-md shadow-2xl p-6 flex flex-col gap-4 w-[720px] transition-all duration-300 ${isImmersiveKpiPanelVisible ? 'rounded-l-2xl' : 'rounded-2xl'}`}>
                  ...
                  <DrillControls layout="immersive" {...{settings, onSettingsChange, currentDrill, setCurrentDrill, availableDrills, selectedDeviceId, setSelectedDeviceId, videoDevices, onNavigate }} />
              </div>
        ...
        <div className="hidden lg:flex lg:w-full lg:max-w-sm flex-shrink-0 flex-col gap-6 bg-dd-surface p-6 rounded-2xl">
          {availableDrills.length > 0 ? (
            <>
              ...
              <DrillControls layout="dashboard" {...{settings, onSettingsChange, currentDrill, setCurrentDrill, availableDrills, selectedDeviceId, setSelectedDeviceId, videoDevices, onNavigate }} />
