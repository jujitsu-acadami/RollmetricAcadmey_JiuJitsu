      ...
      {error ? (
        <div className="flex flex-col items-center gap-6 text-center">
            ...
            {onGoBack && (
              <button
                onClick={onGoBack}
                className="mt-4 bg-dd-surface text-dd-muted hover:text-dd-text py-3 px-6 rounded-lg font-semibold text-lg hover:bg-dd-surfaceAlt transition-colors border border-dd-border"
              >
                Return to Home
              </button>
            )}
        ...
        <div className="flex flex-col items-center gap-6">
          <svg className="animate-spin h-10 w-10 text-dd-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <div className="h-8 flex items-center">
            <p className="text-xl sm:text-2xl text-dd-muted w-72 text-center">
              {displayText}
            </p>
          </div>
        </div>
